...
namespace g_game
{
	...
	namespace g_draw
	{
		...
		void draw_crosshair()
		{
			...
			g_draw::draw_line(ImVec2((center.x), (center.y) - crosshair_size), ImVec2((center.x), (center.y) + crosshair_size), color::draw_crosshair, 1.5f);
			g_draw::draw_line(ImVec2((center.x) - crosshair_size, (center.y)), ImVec2((center.x) + crosshair_size, (center.y)), color::draw_crosshair, 1.5f);
		}
	...
	void main_loop(ImFont* font)
	{
		...
		if (sdk::in_game() > 0 && globals::max_player_count >= 2)
		{
			...
			for (int i = 0; i < globals::max_player_count; ++i)
			{
				...
				{
					int dist = sqrt(pow(local_player.get_pos().x - player_info.get_pos().x, 2) + pow(local_player.get_pos().y - player_info.get_pos().y, 2)) * 0.0254f;
					//int dist = sqrt(pow(local_player.get_pos().x - local_player.get_pos().x, 2) + pow(local_player.get_pos().y - local_player.get_pos().y, 2)) * 0.0254f;
					sdk::DrawRadarPoint(&player_info, &local_player, radarVars::v2RadarNormalLocation.x, radarVars::v2RadarNormalLocation.y, radarVars::v2RadarNormalSize.x, radarVars::v2RadarNormalSize.y, radarVars::Draw, dist);
				}
				...
				if (globals::b_line)
				{
					...
					g_draw::draw_line(ImVec2(origin.x / 2, origin.y), ImVec2(ScreenPos.x, ScreenPos.y), color, 1.f);
				}
				...
				if (globals::b_names)
				{
					...
					g_draw::draw_sketch_edge_text(font, name, ImVec2(ScreenPos.x, BoxSize.y - 70 / 2), globals::b_namesize, color, true);
				...
				if (globals::b_distance)
				{
					...
					distance_string += "m ]";
					g_draw::draw_sketch_edge_text(font, distance_string, ImVec2(ScreenPos.x, ScreenPos.y + 5 / 2), globals::b_distancesize, color, true);
				}
